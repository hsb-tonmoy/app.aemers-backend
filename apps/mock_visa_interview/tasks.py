...
def concatenate_audio_pydub(audio_clip_paths, output_path, verbose=1):
    ...
    audio_clip_paths = tqdm(
        audio_clip_paths, "Reading audio file") if verbose else audio_clip_paths
    ...
    final_clip = clips[0]
    range_loop = tqdm(list(range(1, len(clips))),
                      "Concatenating audio") if verbose else range(1, len(clips))
    for i in range_loop:
